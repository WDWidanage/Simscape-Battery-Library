function  [dv,SF] = getDistributedVariables(simout, block_names, nvp)
% Get the distibuted variables of TECMD, TSPMe and TSPMeA via post
% procesing
%
...
for bb = 1:numel(block_names)
    ...
    if  ismember(block_name,simout_block_names)
        ...
        model_para = getModelParameters(block_path);
        ...
        if ismember(model_type,["TSPMe","TSPMeA"])
            ...
            c = getConcentrations(simout_states,block_path,model_para.(block_name));
            T = getSignal(simout_states,block_path,"T");
            SF = scalingFactors(model_para.(block_name),T);
            
            i = getCurrent(simout_states,block_path,model_para.(block_name),SF);
            phi = getPotentials(simout_states,block_path,model_para.(block_name),SF,i);
        ...
        if ismember(model_type,["TSPMe","TSPMeA"])
            dv.(block_name).SR = getSideReactions(simout_states,block_path,model_para.(block_name),SF,i);
        end
...
function c = getConcentrations(states,block_path,para)
...
cn_hat = getSignal(states,block_path,"cn_hat");
cp_hat = getSignal(states,block_path,"cp_hat");

cen_hat = getSignal(states,block_path,"cen_hat");
ces_hati = getSignal(states,block_path,"ces_hati");
cep_hat = getSignal(states,block_path,"cep_hat");
...
function  i = getCurrent(states,block_path,para,sf)
...
c = getConcentrations(states,block_path,para);
...
etaBV_p = getSignal(states,block_path,"etaBV_p");
...
function phi = getPotentials(states,block_path,para,sf,i)
...
try
    eps_n = getSignal(states,block_path,"eps_n");
catch
    eps_n = para.eps_n;
end
eps_s = para.eps_s;
...
ces_hati = getSignal(states,block_path,"ces_hati");
cep_hat = getSignal(states,block_path,"cep_hat");
etaBV_n = getSignal(states,block_path,"etaBV_n");
etaBV_p = getSignal(states,block_path,"etaBV_p");
cn_hat = getSignal(states,block_path,"cn_hat");
cp_hat = getSignal(states,block_path,"cp_hat");
...
function SR = getSideReactions(states,block_path,para,sf,i)
...
x = getElectrodeSeperatorDims(para);
phi = getPotentials(states,block_path,para,sf,i);
...
SR.Jsei_hat = getSignal(states,block_path,"Jsei_hat");
SR.JLi_hat = JLi_hat;
...
function signal = getSignal(states,block_path,signal_str)
arguments
    states
    block_path
    signal_str
